...
-   **State Management:** Utilizes Zustand for efficient client-side state management, particularly for the form builder.
...
*   **`src/app`**: This is the core Next.js App Router directory, organizing the application into various routes and components.
    ...
    *   **`page.tsx`**: The public-facing landing page of the application.
*   **`src/components`**: Reusable UI components.
    *   **`custom`**: General-purpose custom components (e.g., `inputs-wrapper.tsx`, `logo.tsx`).
    *   **`form`**: Components specifically for form rendering and configuration (e.g., `NormalInputField.tsx`, `NormalFieldConfigs.tsx`).
...
*   **`src/store/builderStore.ts`**: Implements client-side state management using Zustand for the form builder's dynamic state.
